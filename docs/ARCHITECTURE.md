...
Local listening uses 16 kHz mono PCM16 (cloud Live retains 24 kHz). Each audio source has a resident native worker running the Paraformer bilingual streaming INT8 recognizer plus Silero VAD on CPU. Segments are bounded at 12 seconds. Paraformer starts its streaming decoder with up to 500 ms of buffered VAD onset audio. `LocalLiveProvider` gates delegations with local Chinese/English question rules, waits for stable silence, and reports VAD activity so the coordinator holds pending work while speech resumes. Paraformer maintains live encoder/decoder caches and emits replaceable partial text; partials appear in TranscriptStore and expand the overlay but never enter ConversationState, persistence or analysis requests. VAD finalization emits one stable fragment and clears the preview. The local question heuristic is not equivalent to Live semantic comprehension. The existing manual hotkey remains the fallback.
...
`LiveProvider`, `EmbeddingProvider`, and `ReasoningProvider` define the boundaries. `ReasoningProviderFactory` selects OpenAI Responses (shared or separate Key), DeepSeek/Qwen/GLM/Kimi Chat Completions, or a custom compatible Chat Completions endpoint. V1.2 adds local listening and embeddings; selecting both with DeepSeek does not require or initiate a read of the OpenAI credential on startup. Qwen emits `enable_thinking`; GLM/Kimi emit `thinking.type`; DeepSeek retains its thinking/effort mapping. Model default omits overrides and custom endpoints receive no vendor-specific thinking fields. New presets use separate endpoint-scoped Keychain identities. Only `delta.content` is displayed; `reasoning_content` is ignored. Error bodies are not echoed.
...
Credentials stay outside Codable settings. Existing Live identity is preserved. Separate OpenAI and DeepSeek identities cannot fall back to the Live key. Compatible and Qwen/GLM/Kimi preset credentials are additionally scoped to their canonical endpoint. Changing a selected service invalidates the cached analysis credential; revision IDs discard delayed reads from a previous selection. Requests snapshot the selected provider and credential before streaming. Missing OpenAI embeddings can degrade to lexical retrieval while a separately configured analysis provider remains usable.
...
## V1.4 preference migration

Retired listening raw value `local` decodes to `paraformer`; other settings survive. The sentence recognizer, installer entry and native inference route are removed, but existing weights are not deleted. `transcriptFontSize` and `answerFontSize` default independently and clamp to 11–28 pt on decode. SwiftUI applies sizes to preview/final transcripts and answer content; existing measured content heights preserve reflow and scrolling.
