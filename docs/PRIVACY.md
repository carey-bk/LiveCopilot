...
Qwen, GLM and Kimi presets keep keys separate by provider and canonical endpoint. Switching a region or URL never inherits the previous destination’s credential. Model IDs, thinking mode and text-size preferences contain no secrets. Retiring SenseVoiceSmall does not delete previously downloaded model files.
