...
- Services → Live service → **Local · Paraformer-zh-streaming** for live Chinese/English captions (approximately 238 MB). This bilingual export supports both languages; it is not an English accuracy guarantee. **Local · SenseVoiceSmall + VAD** remains available for automatic zh/en/ja/ko/yue recognition with captions after pauses.
- For SenseVoice, download the speech pack if missing (approximately 164 MB download, 240 MB installed).
